# Day 032 — 스무고개로 주식 맞추기: 결정 트리 & 랜덤 포레스트

> "주가가 높아?" "거래량이 많아?" 질문을 반복해서 주식을 분류합니다.
...
features = ['ret', 'ma5', 'ma20', 'vol_ma5', 'high_ma']
...
feature_names = ['수익률', '5일평균', '20일평균', '5일거래량', '평균이상여부']
...
```python
# 과제: 현대차 주가에 랜덤 포레스트 적용
# 1) 현대차 주가 300일치 만들기 (시작 200,000원)
# 2) 특성: 수익률, 5일평균, 20일평균, 거래량평균
# 3) 트리 50개, 100개, 200개로 각각 학습 후 정확도 비교
# 4) 가장 중요한 특성 출력

np.random.seed(7)
hyundai = 200000 + np.cumsum(np.random.randn(300) * 2000)
# 나머지를 채워보세요!
```
...
| 챕터 | 주제 | 실행 방법 |
|------|------|---------|
| [chapter07](../chapters/chapter07/practice.py) | 결정 트리 | `cd chapters/chapter07 && python practice.py` |
| [chapter08](../chapters/chapter08/practice.py) | 랜덤 포레스트 | `cd chapters/chapter08 && python practice.py` |
