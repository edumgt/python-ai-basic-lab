...
def run() -> dict:
    ...
    try:
        ...
        pattern = re.compile(r"<tr>\s*<td>([^<]+)</td>\s*<td>([\d,]+)</td>\s*</tr>", re.IGNORECASE)
        for date, close_text in pattern.findall(html):
            rows.append({"date": date.strip(), "close": int(close_text.replace(",", ""))})
            if len(rows) == 5:
                break
