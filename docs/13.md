# LLM 파인튜닝 실험실: 거대 모델을 내 입맛에 맞게 바꾸기
...
### Instruction Tuning 처리 흐름

```mermaid
flowchart TD
    A["지시 데이터셋\n{instruction, input, output} 쌍"] --> B["프롬프트 템플릿 적용\n'지시에 따라 응답하세요...'"]
    B --> C["토크나이저 인코딩\n텍스트 → 토큰 ID"]
    C --> D["언어 모델\n(Full / LoRA / QLoRA)"]
    D --> E["다음 토큰 예측\n교차 엔트로피 손실 계산"]
    E --> F["손실 역전파\n파라미터 업데이트"]
...
flowchart TD
    Trans["Transformer\n(2017, Vaswani 외)"]
    GPT["GPT 시리즈\n(2018~2023, OpenAI)"]
    InstrGPT["InstructGPT\n(2022, OpenAI)\nRLHF 정렬 학습"]
    IT["Instruction Tuning\n(2021~, Stanford·Google)"]
    LLaMA["LLaMA·Gemma·Mistral\n(2023~, Meta·Google)"]
    LoRA["LoRA\n(2021, Microsoft Research)\n저랭크 어댑터"]
    QLoRA["QLoRA\n(2023, UW·HuggingFace)\n4비트 양자화 + LoRA"]
    PEFT["PEFT 계열\n(Adapter·Prefix·LoRA 등)"]

    Trans -->|"자기회귀 생성"| GPT
    GPT -->|"인간 피드백 정렬"| InstrGPT
    InstrGPT -->|"지시-응답 데이터 학습"| IT
    GPT -->|"오픈소스 대형 모델"| LLaMA
    LLaMA -->|"효율적 파인튜닝"| PEFT
    PEFT -->|"저랭크 행렬 근사"| LoRA
    LoRA -->|"4비트 양자화 결합"| QLoRA

    style Trans fill:#e1bee7
    style GPT fill:#e1bee7
    style InstrGPT fill:#f8bbd0
    style IT fill:#f8bbd0
    style LLaMA fill:#f8bbd0
    style PEFT fill:#c8e6c9
    style LoRA fill:#c8e6c9
    style QLoRA fill:#d4e6ff
```
