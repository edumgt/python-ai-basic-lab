...
| 말 | 쉬운 뜻 | 신경망에서의 느낌 |
|------|------|------|
| **스칼라(Scalar)** | 숫자 하나 | `0.8`, `-0.1`, `2.3` 같은 한 개 값 |
| **벡터(Vector)** | 숫자 여러 개를 한 줄로 묶은 것 | `[수익률, 거래량, 이평선]` 같은 입력 묶음 |
| **선형변환(Linear Transformation)** | 벡터에 가중치를 적용해 다른 점수나 다른 벡터로 바꾸는 계산 | 입력 벡터에 가중치 행렬을 곱해 은닉층 점수를 만드는 과정 |

#### 스칼라와 벡터

- **스칼라**는 숫자 하나입니다.
- **벡터**는 숫자 여러 개를 묶은 한 덩어리입니다.
...
주식 AI에서는 한 종목의 여러 특성(수익률, 거래량, 이동평균)을 묶어서  
**입력 벡터**로 넣는다고 생각하면 쉽습니다.

#### 선형변환

선형변환은 입력 벡터에 가중치를 적용해서  
**새로운 점수**나 **새로운 벡터**를 만드는 계산입니다.
...
즉, 선형변환은  
**"입력 벡터를 다음 층이 이해할 수 있는 새로운 숫자들로 바꾸는 과정"** 입니다.
...
### 뉴런 안에서 실제로 무슨 계산을 할까요?

| 말 | 쉬운 뜻 | 아주 짧은 식 |
|------|------|------|
| **가중합** | 입력마다 중요도를 곱해서 더한 값 | `x1*w1 + x2*w2 + ...` |
| **편향** | 기준점을 살짝 옮겨주는 값 | `가중합 + b` |
| **오차** | 예측이 실제와 얼마나 다른지 | `예측 - 정답` |
| **비용** | 오차를 숫자 하나로 요약한 값 | `오차를 제곱해서 평균` 등 |

#### 1. 가중합(Weighted Sum)

가중합은 입력값에 **중요도 점수(가중치)**를 곱해서 모두 더한 값입니다.
...
```python
0.8 * 0.6 + 0.3 * 0.2 = 0.54
```

즉, 뉴런은 먼저 "어떤 입력을 얼마나 중요하게 볼까?"를 반영해 점수를 만듭니다.

#### 2. 편향(Bias)

편향은 가중합에 **마지막으로 더해주는 보정값**입니다.

```python
z = 가중합 + 편향
```

예를 들어 가중합이 `0.54`이고 편향이 `-0.10`이면:

```python
z = 0.54 + (-0.10) = 0.44
```

> 쉬운 비유: 편향은 시험 점수에 더해지는 **기본 가산점 또는 감점** 같은 느낌입니다.  
> 가중합이 같아도 편향이 바뀌면 최종 판단 기준이 달라집니다.

#### 2-1. 시그모이드 함수(Sigmoid)

시그모이드 함수는 뉴런이 만든 점수 `z`를  
**0과 1 사이 값**으로 바꿔주는 함수입니다.
...
즉, "상승할 가능성이 약 61%"처럼 읽을 수 있습니다.

#### 2-2. 활성화 함수(Activation Function)

활성화 함수는 뉴런의 최종 출력을 결정하는 함수입니다.

대표 예시는:

| 함수 | 쉬운 느낌 | 자주 쓰는 곳 |
|------|------|------|
| **Sigmoid** | 0~1 확률처럼 바꿔줌 | 이진 분류 출력층 |
| **ReLU** | 음수는 0, 양수는 그대로 | 은닉층에서 매우 자주 사용 |
| **tanh** | -1~1 사이로 눌러줌 | 시계열, RNN 계열 설명에서 자주 등장 |

한 줄 감각으로는:

- **선형변환**이 점수를 만들고
- **활성화 함수**가 그 점수를 해석하기 쉬운 출력으로 바꿉니다.

#### 3. 오차(Error)

오차는 **예측값과 실제 정답의 차이**입니다.
...
부호는 방향을 알려주고, 크기는 얼마나 틀렸는지 보여줍니다.

#### 4. 비용(Cost, Loss)

비용은 오차를 바탕으로 "지금 모델이 전체적으로 얼마나 틀리고 있나?"를 숫자 하나로 요약한 값입니다.

실습에서는 가장 쉬운 형태로 이렇게 볼 수 있습니다:
...
오차가 작아질수록 비용도 함께 작아집니다.  
그래서 신경망 학습의 목표는 **비용을 계속 줄이는 것**입니다.
...
한 개 뉴런만 떼어서 보면 순전파는 보통 이렇게 이해하면 됩니다.

```python
가중합 = x1*w1 + x2*w2
z = 가중합 + b
예측 = sigmoid(z)   # 또는 ReLU, tanh 등
```

즉, **가중합 → 편향 더하기 → 활성화 함수 통과 → 예측값 만들기**가 순전파의 가장 작은 단위입니다.
