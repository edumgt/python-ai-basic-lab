...
def walk_forward_evaluate(
    ...
    while start + train_days + test_days <= n:
        ...
        window_id = start // test_days  # 몇 번째 창인지
...
for name, prob in prob_dict.items():
    ...
    ret = df_test["close"].pct_change(1).shift(-1).fillna(0) * (prob >= 0.55)
    bt_metrics[name] = calc_metrics(cum, ret)

# 앙상블 백테스트
cum_ens = backtest(df_test, ensemble_prob)
ret_ens = df_test["close"].pct_change(1).shift(-1).fillna(0) * (ensemble_prob >= 0.55)
