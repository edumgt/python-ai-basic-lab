# 종합 미니 프로젝트: 작은 AI 탐정 보고서 만들기

> 이제는 문서를 읽고, 웹앱에서 실험하고, 결과를 한 문장으로 설명하는 작은 프로젝트를 완성합니다.
...
flowchart TD
    ...
    LSTM["LSTM\n(1997)"]
    Trans["Transformer\n(2017, Vaswani 외)"]
    BERT["BERT / KoBERT / FinBERT\n(2018~2019)"]
    TST["PatchTST·TFT·iTransformer\n(2019~2023)"]
    MC["몬테카를로 시뮬레이션\n(1946, von Neumann)"]
    ...
    style BERT fill:#f8bbd0
    style TST fill:#f8bbd0
...
```mermaid
flowchart TD
    A["입력 파라미터\n(기대 수익률 μ, 변동성 σ, 기간)"] --> B["일별 수익률 분포 설정\nr_t ~ N(μ_daily, σ_daily²)"]
    B --> C["경로 1 시뮬레이션\n가격_t = 가격_{t-1} × exp(r_t)"]
    B --> D["경로 2 시뮬레이션\n..."]
    B --> E["경로 N 시뮬레이션\n(N = 1,000~10,000회)"]
    C --> F["최종 가격 분포\n수천 개 시나리오 집계"]
