# Day 033 — 팀플레이 예측: XGBoost & LightGBM (코스피 실전)

> 여러 모델이 협력할수록 예측이 더 정확해집니다. 실제 국내 증시 데이터로 확인해봅니다.
...
## 3. 국내 주가 데이터 준비 (FinanceDataReader)
...
try:
    ...
    df_raw = fdr.DataReader('005930', '2019-01-01', '2024-12-31')
...
df['ret_20']    = df['close'].pct_change(20)          # 20일 수익률
df['ma5']       = df['close'].rolling(5).mean()
df['ma20']      = df['close'].rolling(20).mean()
df['vol_ma5']   = df['volume'].rolling(5).mean()
df['vol_ratio'] = df['volume'] / df['vol_ma5']        # 거래량 평균 대비 비율
df['cross']     = (df['ma5'] > df['ma20']).astype(int) # 골든크로스 여부
df['volatility']= df['ret'].rolling(10).std()          # 10일 변동성

# 내일 오를지(1) 내릴지(0)
df['target'] = (df['close'].shift(-1) > df['close']).astype(int)
df = df.dropna()

features = ['ret', 'ret_5', 'ret_20', 'ma5', 'ma20', 'vol_ratio', 'cross', 'volatility']
...
```python
feature_names = ['수익률', '5일수익률', '20일수익률', '5일평균', '20일평균', '거래량비율', '골든크로스', '변동성']
...
plt.xlabel('중요도')
plt.title('삼성전자 Gradient Boosting 특성 중요도')
plt.tight_layout()
...
```python
# 과제: 여러 코스피 종목에 Gradient Boosting 적용
# 1) 삼성전자(005930), 카카오(035720), NAVER(035420) 각각 수집
# 2) 동일한 특성(ret, ret_5, ma5, ma20, vol_ratio, volatility)으로 각각 학습
# 3) 세 종목의 예측 정확도 비교 막대 그래프
# 4) 어느 종목이 가장 예측하기 쉬운지 분석

try:
    import FinanceDataReader as fdr
    tickers = {'삼성전자': '005930', '카카오': '035720', 'NAVER': '035420'}
    stock_data = {}
    for name, code in tickers.items():
        raw = fdr.DataReader(code, '2022-01-01', '2024-12-31')
        stock_data[name] = raw[['Close', 'Volume']].rename(
            columns={'Close': 'close', 'Volume': 'volume'})
except Exception:
    np.random.seed(99)
    stock_data = {
        '삼성전자': pd.DataFrame({'close': 60000 + np.cumsum(np.random.randn(600) * 800),
                                   'volume': np.random.randint(8_000_000, 25_000_000, 600)}),
        '카카오':   pd.DataFrame({'close': 40000 + np.cumsum(np.random.randn(600) * 600),
                                   'volume': np.random.randint(2_000_000, 10_000_000, 600)}),
        'NAVER':  pd.DataFrame({'close': 150000 + np.cumsum(np.random.randn(600) * 2000),
                                   'volume': np.random.randint(1_000_000, 5_000_000, 600)}),
    }
...
| 챕터 | 주제 | 실행 방법 |
|------|------|---------|
| [chapter09](../chapters/chapter09/practice.py) | 앙상블 실습 | `cd chapters/chapter09 && python practice.py` |
| [chapter110](../chapters/chapter110/practice.py) | XGBoost 실습 | `cd chapters/chapter110 && python practice.py` |
